...
public:
	...
	timeval GetConnectedTime() const { return m_tvConnectedTime; }
	...
	char* GetIPString() const {
		static char ipStr[INET_ADDRSTRLEN];  // Buffer to store the IP address string

		// Use inet_ntop to convert the IP address to string
		if (inet_ntop(AF_INET, &(m_Address.sin_addr), ipStr, INET_ADDRSTRLEN) != NULL) {
			return ipStr;
		}
		else {
			return nullptr;  // Handle error, if any
		}
	}
	DWORD GetIP() const { return m_Address.sin_addr.S_un.S_addr; }
	WORD GetRawPort() const { return m_Address.sin_port; }
	...
	timeval GetLastPacketRecvTime() const { return m_tvLastPacketRecvTime; }

	DWORD GetAuthKey() const { return m_dwAuthKey; }
