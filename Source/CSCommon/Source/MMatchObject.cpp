...
bool MMatchObject::IsEnabledRespawnDeathTime(unsigned int nNowTime)
{
	...
	// Check if pBuffSummary is null to avoid crashes
	if (pBuffSummary == nullptr)
	{
	...
	if (nNowTime < m_nDeadTime)
	{
		return false;  // If the current time is before death time, it's invalid
	}

	// Simplify the condition by returning the result of the comparison
	return (nNowTime - m_nDeadTime) > (nDelayAfterDying - 500);
}
