...
void MMatchServer::StandbyClanList(const MUID& uidPlayer, int nClanListStartIndex, bool bCacheUpdate)
{
	...
	for (int i = 0; i < nRealCount; i++)
	{
		...
		if (pLadderGroup->GetPlayerCount() > 0)
		{
			...
			size_t playerCount = pLadderGroup->GetPlayerCount();
			if (playerCount > SHRT_MAX) {
				mlog("Warning: Player count exceeds the limit of short.\n");
				playerCount = SHRT_MAX;  // Handle overflow as needed, e.g., clamp to SHRT_MAX
			}
			pNode->nPlayers = static_cast<short>(playerCount);
