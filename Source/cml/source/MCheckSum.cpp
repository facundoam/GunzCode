...
void MCheckSums::AddCheckSum(int nCheckSum, const char* szName, MCheckSumType t)
{
	_ASSERT(m_pCurCheckSum != NULL); // Ensure InitCheckSum() was called beforehand.

	MSUBCHECKSUM* pCSL = new MSUBCHECKSUM;

	// Use strcpy_s to ensure the name fits in the buffer safely
	if (szName == NULL) {
		pCSL->szName[0] = '\0';  // Set empty string if name is NULL
	}
	else {
		// Ensure szName fits into pCSL->szName buffer, assuming it has a fixed size (e.g. 256)
		strcpy_s(pCSL->szName, sizeof(pCSL->szName), szName);
	}

	pCSL->nCheckSum = nCheckSum;
	pCSL->nType = t;

	// Insert the new checksum into the current list
	m_pCurCheckSum->insert(m_pCurCheckSum->end(), pCSL);

	// Update the current checksum value
