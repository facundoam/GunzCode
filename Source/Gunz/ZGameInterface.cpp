...
static bool GetItemDescStr(string& str, DWORD nItemID) {
	...
	if (pItemDesc->m_nResLevel.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "ResLevel: %d", pItemDesc->m_nResLevel.Ref());
	...
	if (pItemDesc->m_nMaxBullet.Ref()) {
		if (bAdd) str += " / ";
		sprintf(temp, "MaxBullets: %d", pItemDesc->m_nMaxBullet.Ref());
